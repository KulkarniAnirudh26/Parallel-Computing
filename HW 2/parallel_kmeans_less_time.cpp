...
int main(int argc, char *argv[]){
	...
	for (int i = 0; i < iterations ; i++){
		...
		//Sum1[6][NUM_THREADS] = 0;
		//NumelementsCluster[6][NUM_THREADS] = 0;
		//int Sum1[6][NUM_THREADS] = {0};
		//int NumelementsCluster[6][NUM_THREADS] = {0};
		//Create threads 
		...
		for (int k = 0; k < 6; k++){
			...
			for (int p = 0; p < NUM_THREADS; p++){
				...
				len = len + accumulate(NumelementsCluster[k].begin(), NumelementsCluster[k].end(),0);
				//std::cout << Sum1[k][p] << std::endl;//len + NumelementsCluster[k][p];
			}
			mean[k] = Sum1K/len;
		}
		//clusters.clear(); 
